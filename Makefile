...
CLEANFILES=$(STAMPFILES) $(SVGFILES:.svg=.pdf) $(DIAFILES:.dia=.pdf) $(TEXFILES:.tex=.pdf) $(TEXFILES:.tex=.synctex.gz) $(TEXFILES:.tex=.out) $(TEXFILES:.tex=.tdo) $(shell basename $(CURDIR)).zip $(shell basename $(CURDIR)).tar.xz
...
%.dia.stamp: %.dia
	...
	epstopdf $(subst .dia.stamp,.eps,$@) --outfile=$(subst .dia.stamp,.pdf,$@)
	@touch $@
